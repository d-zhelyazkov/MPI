...
class ImgReconstructProcessFactory {

public:
    static ImgReconstructProcess* construct(const MPI_Comm& communicator) {
        ...
        if (rank == MAIN_PROC) {
            return new MainImgReconstructProcess(communicator, rank, prevRank, nextRank);
        }
        else {
            return new SecondaryImgReconstructProcess(communicator, rank, prevRank, nextRank);
        }
    }
};